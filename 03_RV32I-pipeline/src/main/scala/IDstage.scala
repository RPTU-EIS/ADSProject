...
class ID () extends Module {
  val io = IO(new Bundle {
    ...
    val XcptInvalid = Output(UInt(1.W))
  })
  ...
  io.XcptInvalid := 1.U // Default to invalid instruction, will be cleared for valid instructions
    ...
    switch(funct3) {
      is("b000".U) { // ADD/SUB
        when(funct7 === "b0000000".U) { // ADD
          io.uop := ADD.asUInt
          io.XcptInvalid := 0.U // Valid instruction

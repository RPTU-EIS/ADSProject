...
for (i <- 0 until numWays) {
  ...
  val hit = Wire(Bool())
  //check if there is a hit in way 0 or way 1
when(wayHits(0) || wayHits(1)) {
  hit := true.B
}.otherwise {
  hit := false.B
}
  ...
  */
  val hitWay = Mux(wayHits(0), 0.U, 1.U)
...
*/
  ...
  when(hit) { // ** Is io.update necesary?
    ...
    when(io.mispredicted) {
      ...
      .otherwise { btb(index)(hitWay).predictor := 0.U }
    }.otherwise {
      when(predState === 0.U) { btb(index)(hitWay).predictor := 1.U }
